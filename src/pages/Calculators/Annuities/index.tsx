...
import {
  ...
  buildVariantSteps,
  type AnnuityType,
  type AnnuityVariantResult,
} from '../../../lib/annuitiesSpecial'
...
const TIPOS = [
  { value: 'ordinaria', label: 'Ordinaria (vencida)' },
  { value: 'anticipada', label: 'Anticipada' },
  { value: 'diferida', label: 'Diferida' },
  { value: 'perpetua', label: 'Perpetua' },
]

const TIPO_LABEL: Record<AnnuityType, string> = {
  ordinaria: 'ordinaria (vencida)',
...
export default function AnnuitiesPage() {
  ...
  // Tipo de anualidad (especiales) — solo en modo experto
  const [tipo, setTipo] = useState<AnnuityType>('ordinaria')
  const [defer, setDefer] = useState('3')
  const [variant, setVariant] = useState<AnnuityVariantResult | null>(null)
  ...
  function calculate() {
    setError('')
    // Tipos especiales (anticipada / diferida / perpetua)
    if (tipo !== 'ordinaria') {
      setResult(null)
      try {
        const r = calcAnnuityVariant(mode, tipo, {
          PMT: +PMT,
          PV: +PV,
          iPct: +iPct,
          n: +n,
        ...
        addHistory({
          module: 'Anualidades',
          label: `${TIPO_LABEL[tipo]} ${r.mode} — PV=${fmtCOP(r.PV)}, i=${+iPct}%${tipo === 'perpetua' ? '' : `, n=${r.n}`}`,
          inputs: { PMT: r.PMT, PV: r.PV, i: +iPct, n: r.n, tipo, mode: r.mode, defer: +defer },
          result: { PV: r.PV, FV: r.FV ?? 0, PMT: r.PMT },
        })
      } catch (e: any) {
    ...
    <div className="p-6 lg:p-10 max-w-4xl mx-auto">
      ...
      {uiMode === 'simple' ? (
        ...
        <>
          ...
          <div className="card p-6 mb-6">
            ...
            <p
              className="text-xs mono uppercase tracking-widest mb-2 mt-1"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
              ...
              {tipo === 'diferida' && (
                <Field
                  ...
                  value={defer}
                  onChange={setDefer}
                  hint="Períodos antes del primer pago."
                />
              )}
            </div>
      ...
      {variant && (
        <div className="animate-fade-up mt-2">
          <Callout color="blue">
            <strong>Anualidad {TIPO_LABEL[variant.type]}:</strong> {variant.note}
          </Callout>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-2">
            ...
            />
            <ResultCard label="Cuota PMT" value={fmtCOP(variant.PMT)} />
            <ResultCard label="Valor presente PV" value={fmtCOP(variant.PV)} />
            <ResultCard
              label="Valor futuro FV"
              value={variant.FV == null ? '∞ (no aplica)' : fmtCOP(variant.FV)}
            />
          </div>

          <Callout color="accent">
            <strong>En palabras: </strong>
            {variant.type === 'perpetua' ? (
              <>
                Una serie de pagos de {fmtCOP(variant.PMT)} que continúa para siempre, a una tasa de{' '}
                {fmtPctShort(variant.iPct)} por período, vale hoy {fmtCOP(variant.PV)}.
              </>
            ) : (
              <>
                Una anualidad {TIPO_LABEL[variant.type]} de {fmtCOP(variant.PMT)} durante{' '}
                {fmtNumber(variant.n, 0)} períodos
                {variant.type === 'diferida' ? ` (con ${variant.defer} de gracia)` : ''} a{' '}
                {fmtPctShort(variant.iPct)} por período tiene un valor presente de{' '}
                {fmtCOP(variant.PV)}
                {variant.FV != null ? ` y un valor futuro de ${fmtCOP(variant.FV)}` : ''}.
              </>
            )}
          </Callout>

          <StepByStep
            steps={buildVariantSteps(variant)}
            contexto={`anualidad ${TIPO_LABEL[variant.type]}`}
          />
        </div>
      )}
