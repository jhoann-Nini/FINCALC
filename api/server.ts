...
const server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
  ...
  // ── Endpoint de explicación de pasos ───────────────────────────
  if (pathname === '/api/explain' && req.method === 'POST') {
    await handleVercelStyle(explainHandler);
    return;
  }
  ...
  res.end(JSON.stringify({
    ...
    availableEndpoints: ['GET /api/health', 'POST /api/chat', 'POST /api/interpret', 'POST /api/explain']
  }));
});
